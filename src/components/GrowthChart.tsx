// Growth charts: WHO weight-for-age at corrected age (live, primary) + Fenton 2013
// preterm chart (historical view of early weights, PMA <= 50 weeks).
// Implemented by workstream W5 (Phase 3). Reads Dexie `weights` + `settings` directly —
// no props.
...
import { correctedAgeMonths, postmenstrualAgeWeeks } from '../growth/age';
...
export function GrowthChart() {
  const [view, setView] = useState<'who' | 'fenton'>('who');
  ...
  const latestCorrectedAge = correctedAgeMonths(dob, gestWeeks, gestDays, latest.date);
  const { L: latestL, M: latestM, S: latestS } = whoWfaBoysAt(latestCorrectedAge);
  ...
  // ---- WHO chart data: percentile curves (0-24 corrected months) + logged weights ----
  ...
  const whoScatter = weights
    .map((w) => ({
      month: correctedAgeMonths(dob, gestWeeks, gestDays, w.date),
  ...
  return (
    <>
      <div className="choices" style={{ marginTop: 8 }}>
        <button type="button" className={`chip ${view === 'who' ? 'on' : ''}`} onClick={() => setView('who')}>
          WHO (0–24 mo corrected)
        </button>
      ...
      <div className="entry" style={{ padding: '11px 4px' }}>
        <div className="body">
          <div className="title">
            {latestKg.toFixed(1)} kg — {ordinal(latestPercentile)} percentile (z {latestZ >= 0 ? '+' : ''}{latestZ.toFixed(2)}) at {latestCorrectedAge.toFixed(1)} mo corrected
          </div>
      ...
      <div style={{ width: '100%', height: 360, marginTop: 8 }}>
        {view === 'who' ? (
          <ResponsiveContainer>
            <ComposedChart data={whoCurve} margin={{ top: 8, right: 12, bottom: 32, left: 0 }}>
              <CartesianGrid stroke="var(--line-soft)" />
              <XAxis
                dataKey="month" type="number" domain={[0, 24]}
                ticks={[0, 3, 6, 9, 12, 15, 18, 21, 24]}
                label={{ value: 'Corrected age (months)', position: 'bottom', offset: 12, fontSize: 12 }}
                tick={{ fontSize: 11 }}
              />
              <YAxis
                ...
                tick={{ fontSize: 11 }}
              />
              <Tooltip formatter={(v) => `${Number(v).toFixed(2)} kg`} labelFormatter={(m) => `${Number(m).toFixed(1)} mo corrected`} />
